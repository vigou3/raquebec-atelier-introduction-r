...
routes <- read.csv("https://raw.githubusercontent.com/
...
# Variable selection 
# We will not use the typeAirport and Source variables since we only want to 
# analyse air transport market
# We can also discard the country variable because we already filtered on 
# Canadian airports
airportsCanada <- subset(airportsCanada, 
                         select = -c(country, typeAirport, Source ))

# As seen in the summary, we do not have the IATA code of 27 airports
subset(airportsCanada, is.na(IATA), 
       select = c("airportID","name","IATA","ICAO"))
# 82% of the time, the IATA code corresponds to the last three characters of 
# the ICAO code
...
library(sp)
library(rgdal)
tz_world.shape <- readOGR(dsn = paste(path,"/ref/tz_world_2",sep = ""),
                          layer = "combined_shapefile")
unknown_tz <- subset(airportsCanada,is.na(tzFormat),
                     c("airportID","name","longitude","latitude"))
...
prov_terr.shape <- readOGR(dsn = paste(path,"/ref/prov_terr",sep = ""),
                           layer = "gpr_000b11a_e")
unknown_prov <- subset(airportsCanada,
                       select = c("airportID","city","longitude","latitude"))
...
airportsCanada <- subset(airportsCanada, 
                         select = -c(timezone, DST, tzFormat, city ))
summary(airportsCanada)

# Rename of the merged variables
# install.packages("plyr")
library(plyr)
airportsCanada <- rename(airportsCanada, 
                         c("tzMerged" = "tzFormat", "provMerged" = "province"))
summary(airportsCanada)
...
# Since almost all routes are direct, we dont need the codeshare 
# and stops variables
...
(mapPoints <- ggmap(map) + geom_point(data = airportsCoord,
                                      aes(lon,lat),alpha = 0.5))
...
(mapRoutes <- mapPoints + 
    geom_segment(data = routesCoord,
                 aes(x = lonBeg,
                     y = latBeg,
                     xend = lonEnd,
                     yend = latEnd),alpha = 0.5))

# Calculation of a standard trafic index based on the number of arrivals 
# and departures
...
combinedIndexTable <- data.frame(IATA,as.vector(totalFlights),
                                 as.vector(combinedIndex),
                                 row.names = NULL)
...
mapPoints <- 
  ggmap(map) + 
  geom_point(data = TraficData,aes(x = lon,y = lat,size = size),
             alpha = 0.5,shape = 16)
(mapTraffic <-  mapPoints + scale_size_continuous(range = c(0, 20),
                                                  name = "Trafic Index"))
...
download.file(url, destfile = paste(path,"/ref/worldRoutes.gpx",sep = ""), 
              method = "wget")
worldRoutes <- readOGR(paste(path,"/ref/worldRoutes.gpx",sep = ""), 
                       layer = "tracks")
...
descriptions <- paste("<b><FONT COLOR=#31B404> Airport Details</FONT></b> <br>",
                    "<b>IATA: <a href=",markersWeb,">",
                    markersData$IATA,"</a></b><br>",
                    "<b>Name:</b>",markersData$name,"<br>",
                    "<b>Coord.</b>: (",markersData$longitude,",",
...
(mapTraffic <- leaflet(worldRoutes) %>%
    ...
    addCircleMarkers(stroke = FALSE,data = TraficData, 
                     ~as.numeric(paste(longitude)), 
                     ~as.numeric(paste(latitude)),
                     color = "black", fillColor = "green",
                     radius = ~as.numeric(paste(combinedIndex))*30, 
                     opacity = 0.5) %>%
    addAwesomeMarkers(data = markersData, 
                      ~as.numeric(paste(longitude)), 
                      ~as.numeric(paste(latitude)), 
                      popup = descriptions,icon = icons))
...
library(htmlwidgets)
saveWidget(mapTraffic, paste(path,"/ref/leafletTrafic.html",sep = ""), 
           selfcontained = TRUE)