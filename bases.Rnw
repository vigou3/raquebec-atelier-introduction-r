...
\begin{frame}[fragile=singleslide]
  ...
  \item Fonctions de base pour créer des vecteurs
    ...
    \item \verb|numeric| (vecteur numérique)
    \item \verb|logical| (vecteur booléen)
    \item \verb|character| (vecteur de chaines de caractères)
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Nombre variable d'arguments avec \texttt{'\alert{...}'}
  \end{itemize}
