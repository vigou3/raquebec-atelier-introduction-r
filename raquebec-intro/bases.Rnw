...
\begin{frame}[fragile=singleslide]
  ...
  \begin{itemize}
  \item Opérateurs de base définis pour les vecteurs
  \item Vecteur simple: tous les éléments du même \alert{mode}
  \item Fonctions de base pour créer des vecteurs
    \begin{itemize}
    \item \verb|c| (concaténation);
    \item \verb|numeric| (vecteur de mode \texttt{numeric})
    \item \verb|logical| (vecteur de mode \texttt{logical})
    \item \verb|character| (vecteur de mode \texttt{character})
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Fonction est un \alert{objet} comme un autre
    ...
    \item définition de nouvelles fonctions
    \item fonction comme argument d'une fonction
    \item fonction comme valeur retournée par une fonction
    \end{itemize}
  \item Nombre variable d'arguments avec \texttt{'...'}
  \item Définition de fonctions \alert{anonymes}
  \end{itemize}
